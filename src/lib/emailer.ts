...
export async function sendConfirmationEmailAsync(params: {
  ...
  const { toEmail, toName, ticketId, tierName } = params;
  ...
  try {
    ...
    const htmlBody = `
      ...
      <head>
        ...
        <style>
          ...
          .header { border-bottom: 2px solid #e1a10b; padding-bottom: 16px; margin-bottom: 24px; text-align: center; }
          .gold-title { font-size: 20px; font-weight: bold; color: #e1a10b; text-transform: uppercase; margin: 0; }
          .ticket-id { font-family: monospace; font-size: 14px; color: #2b6e59; font-weight: bold; margin-top: 6px; }
      ...
      <body>
        <div class="card">
          ...
          <p>Mostra questa email o il tuo Pass ID (<strong>${ticketId}</strong>) all'ingresso del seminario.</p>
    ...
    console.log(`[EMAIL DELIVERED] Confirmation email sent to ${toEmail}!`);
    return true;
  } catch (err) {
